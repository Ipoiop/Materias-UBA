...
--import Test.HUnit --DESCOMENTAR LAS PRIMERAS DOS LINEAS
...
procRaizTrie (TrieNodo (Just r)  _) = [Just r]
...
foldAT fNil fTree t = case t of
                    Nil -> fNil
                    Tern a h1 h2 h3 -> fTree a (rec h1) (rec h2) (rec h3)
                    where rec = foldAT fNil fTree
...
foldTrie :: (Maybe a -> [b] -> b) ->  ((Char, b) -> b) -> Trie a -> b
foldTrie fTrie fTupla (TrieNodo a hijos) = fTrie a (map (\(ch, TrieNodo t h) ->
                     fTupla (ch, rec (TrieNodo t h))) hijos)
                     where rec = foldTrie fTrie fTupla
...
caminos = foldTrie (\_ rec -> concat rec) (\(c, rec) -> [c] : map ([c] ++) rec)


--Ejercicio 7

palabras :: Procesador (Trie a) String
palabras = foldTrie (\(Just a) rec -> concat rec) (\(c, rec) -> [c] : map ([c] ++) rec)
