...
def avanzarFila(fila: Queue, min: int):
  #implementar función
  sigPer: int = fila.qsize() + 1
  cuentaMin: int = 0
  caja3activo: bool = False
  while cuentaMin <= min:#cambiar los if
    pobre: int
    if(cuentaMin % 4 == 0): #nuevo a la fila
      fila.put(sigPer)
      sigPer = sigPer +1
    if(cuentaMin % 10 == 1 and fila.qsize()>0):  #caja 1
      fila.get()
    if(cuentaMin % 4 == 3 and fila.qsize()>0):   #caja 2
      fila.get()
    if(cuentaMin % 4 == 2 and fila.qsize()>0):   #caja 3
      pobre = int(fila.get())
      caja3activo = True
    if(cuentaMin % 4 == 1 and cuentaMin>=5 and caja3activo == True): #el de caja 3 vuelve a la fila
      fila.put(pobre)
      caja3activo = False
    cuentaMin += 1
